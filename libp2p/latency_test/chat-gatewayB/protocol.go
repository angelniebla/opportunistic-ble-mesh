				...
				if !waiting_ack {
					...
					if sender_user != "you" {
						...
						if kademliaDHT != nil && len(message_full) > 0 {
							...
							if message_parse == "load" {
								if receiver_user != "" {
									recover_value, e := kademliaDHT.GetValue(context.Background(), "/v/"+receiver_user)
									if e != nil {
										log.Printf("not value: %v", e)
										continue
									}
									...
									recover_value, e := kademliaDHT.GetValue(context.Background(), "/v/"+sender_user)
									if e != nil {
										log.Printf("not value: %v", e)
										continue
									}
...
func subscribe(data string) {
	for {
		...
		ch, err := pubsubb.SearchValue(context.Background(), "/v/0x0011")
		...
		*/
		...
		if value == string([]byte("")) {
			//log.Println("Value find is not the search: ", value)
			continue
		}

		go pubsubb.PutValue(context.Background(), "/v/0x0011", []byte(""))

		//log.Println("GOT VALUE: ", value)

		log.Println("ARRIVAL TIME: ", time.Now())

		protocolBridge.forwardMessage(value, "0x0011")
