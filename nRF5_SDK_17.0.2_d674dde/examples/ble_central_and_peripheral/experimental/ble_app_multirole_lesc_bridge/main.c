...
void send_command2 ()
{
  uint8_t * send_cmd = (uint8_t *) ("at+version\r\n");
    for(int i = 0; i < strlen(send_cmd); i++)
    {
...
<<<<<<< HEAD
            if (is_valid && send) {
=======
            if (is_valid && send) { // proccess average values every X seconds not when
                                    // valid event received due to duty cycle limitation
>>>>>>> origin/saadc_v1
